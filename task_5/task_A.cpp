...
class Hashtable {
    ...
    int hash_by_key(Key &key) {
        return (key % this->capacity);
    }

    void reshape(int new_capacity) {
...
public:
    ...
    void insert_item(Key &key, Value &value) {
        ...
        int position = this->hash_by_key(key);
    ...
    void print() {
        ...
        for (int i = 0; i < this->capacity; i++) {
            if (!this->data[i].is_empty) {
                cout << i << " " << this->data[i].key << " " << this->data[i].value << " "  << this->data[i].is_deleted << endl;
            }
        }
